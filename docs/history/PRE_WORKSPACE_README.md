Historical README at baseline 2f6569280ed272940e21f9c3a1af0702be457f15. This is not the current operator guide.
...
- Backend signal, risk, approval-ticket, audit, order, position, and alert `GET` endpoints
  atomically project validated durable saved-workflow simulation lifecycle evidence. Unreached
  downstream stages are durably empty, and all seven resources remain visibly representative only
  when no committed run evidence exists.
