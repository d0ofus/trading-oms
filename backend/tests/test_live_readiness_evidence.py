...
def test_live_readiness_evidence_dashboard_exposes_missing_evidence_only() -> None:
    ...
    assert payload["missing_evidence_count"] == 4
    assert payload["blocking_evidence_count"] == 1
    assert payload["blocking_reason"] == "missing_external_review_and_human_approval"
    ...
    assert [item["status"] for item in payload["evidence_items"]].count("missing") == 4
    assert [item["status"] for item in payload["evidence_items"]].count(
...
def test_live_readiness_evidence_validation_blocks_unsafe_postures() -> None:
    ...
    with pytest.raises(ReadModelError, match="ready_for_final_review evidence must be satisfied"):
        LiveReadinessEvidenceDashboardReadModel(
            dashboard_id="live-readiness-evidence-001",
            evaluated_at="2026-07-08T00:08:00Z",
...
def test_live_readiness_evidence_api_is_read_only_and_safe(monkeypatch: MonkeyPatch) -> None:
    ...
    assert payload == build_demo_operations_read_model().live_readiness_evidence.to_json_dict()
